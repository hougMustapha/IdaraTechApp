    ...
    FB.login(async (fbResult: any) => {
      if (fbResult.authResponse) {
        ...
        this.accountService.loginWithThirdParty(new LoginWithExternal(accessToken, userId, "facebook")).subscribe({
          ...
          error: error => {
            this.sharedService.showNotification(false, "Echec", error.error);
          }  
        })

      } else {
        this.sharedService.showNotification(false, "Échec", "Impossible de se connecté avec votre Facebook");
      }
    })
  }
